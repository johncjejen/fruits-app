  ...
  render(){
    ...
    return(
      <div>
        {name}
        {description}
        <button className="button" onClick={() => this.handleEdit()}>{this.state.editable? 'Submit' : 'Edit'}</button>
        <button className="button" onClick={() => this.props.handleDelete(this.props.fruit.id)}>Delete</button>
      </div>
    )      
  }
}
